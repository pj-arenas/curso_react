      ...
      const addNewPerson = (event) =>{
        ...
        if (!personFind){      
          ...
          .then(response=>{
            ...
            setNewName('')
            setNewNumber('')
            setErrorMessage({message:`Añadido ${newName} `, error: false})
            setTimeout(() => {
          ...
          if(window.confirm(`${personFind.name} ya esta añadido, ¿Quieres remplazar su antuguo numero por este nuevo?`)){
            agendaService.update(personFind.id, newPersonObj).then((response)=>{
              ...
              setNewName('')
              setNewNumber('')
              setErrorMessage({message:`Actualizado ${newName} `, error: false})
              setTimeout(() => {
                setErrorMessage(null)
              }, 5000)
            }).catch((error)=>{
              setErrorMessage({message:`Error: ${error} `, error: true})
              setTimeout(() => {
                setErrorMessage(null)
              }, 5000)
            })
